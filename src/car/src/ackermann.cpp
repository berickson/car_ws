...
Point Ackermann::front_position() const {
  Point p;
  p.x = x + l * cos(heading);
  p.y = y + l * sin(heading) ;
  return p;
...
Point Ackermann::rear_position() const {
  Point p;
  p.x = x;
  p.y = y;
  return p;
...
string Ackermann::to_string() const {
  stringstream ss;
  ss <<  "Ackermann x:" << x << " y:" << y << " heading: " << heading;
  return ss.str() ;
}

void Ackermann::move_relative_to_heading(Point p) {
  this->x += cos(heading) * p.x + sin(heading) * p.y;
  this->y += sin(heading) * p.x + cos(heading) * p.y;
}

  // x is ahead, l is to left, x must be positive
Ackermann::Arc Ackermann::arc_to_relative_location(double x,double y) {
  Arc arc;
  if(fabs(y) > 0.000001) {
...
void Ackermann::move_right_wheel(Angle outside_wheel_angle, double wheel_distance, double new_heading){
...
void test_arc_to_relative_location(double l, double x, double y){
  auto car = Ackermann(0, l, Point(0,0)); // zero width makes like a bicycle, -l puts front wheel at (0,0)
...
void test_move_right_wheel(double l, Angle outside_wheel_angle, double distance) {
  auto car = Ackermann(0, l, Point(0, 0)); // zero width makes like a bicycle
